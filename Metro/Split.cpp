															...
															//returns the corresponding vector of strings
{
	...
	while(i<str.size())
	{
		...
		{
			InsertEnd(splittedString, str.substr(i-k,k)); //add the string between two delimiters (w/o them) to the vector splitted String
			                //(i because str[i-k-1] is a delimiter)	
		}
		else  //that means there is no string
		{	
			
			InsertEnd(splittedString, "");  //we want to know there are two delimiters with nothing between them
		}
		i++;
		k=0; 
	...
	if(str[str.size()-1] = delimiter)
		{
			InsertEnd(splittedString, "");  //if a delimiter is at the end of the string, it will take it into account
